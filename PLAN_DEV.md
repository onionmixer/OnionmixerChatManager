...
## 31. 채팅 표시 설정 및 UI 레이아웃 개선

> 작업일: 2026-04-11

### 31.1 채팅 폰트/스타일/간격 설정

Configuration → General 탭에 채팅 표시 설정을 추가한다.

| 설정 | INI 키 | 기본값 | 범위 |
|------|--------|--------|------|
| Chat Font | `chat_font_family` | 시스템 기본 | QFontComboBox |
| Chat Font Size | `chat_font_size` | 11 | 8~24px |
| Chat Font Bold | `chat_font_bold` | false | QCheckBox |
| Chat Font Italic | `chat_font_italic` | false | QCheckBox |
| Chat Line Spacing | `chat_line_spacing` | 3 | 0~20px |

모든 요소 크기는 `chatFontSize` 기준 비례 계산:
- badge 크기: `fontSize + 2`
- badge 폰트: `max(fontSize - 4, 6)`
- timestamp 폰트: `max(fontSize - 2, 7)`
- 행 높이: `fontSize * 3 + lineSpacing * 2`

### 31.2 채팅 프리뷰

General 탭 하단에 Chat Preview 그룹박스를 배치하여 폰트/크기/스타일/간격 변경 시 Apply 없이 즉시 미리보기 가능. 샘플 메시지 3개 (영어 YouTube, 한국어 CHZZK, 일본어 YouTube).

### 31.3 타임스탬프 표시

Messenger 뷰에서 닉네임 오른쪽에 `yyyy-MM-dd HH:mm:ss` 형식의 타임스탬프를 표시한다. 색상은 `#999999`, 크기는 `max(fontSize - 2, 7)`.

### 31.4 QSplitter 레이아웃

메인 윈도우에 QSplitter를 도입하여 드래그로 영역 크기를 조절할 수 있도록 한다.

- 좌우 Splitter (`m_upperSplitter`, Qt::Horizontal): 채팅 테이블 ↔ 액션 패널 (초기 3:2)
- 상하 Splitter (`m_mainSplitter`, Qt::Vertical): 상단(채팅+액션) ↔ 하단(컴포저+로그) (초기 4:1)
- 컴포저(입력창+전송버튼)는 하단 영역 내 고정 높이(68px)로 배치, splitter 드래그 시 이벤트 로그만 영향받음
- 윈도우 크기/위치 및 splitter 비율은 종료 시 INI `[window]` 그룹에 자동 저장되며, 다음 시작 시 복원
  - `[window]geometry`: `QMainWindow::saveGeometry()` / `restoreGeometry()`
  - `[window]main_splitter`: `m_mainSplitter->saveState()` / `restoreState()`
  - `[window]upper_splitter`: `m_upperSplitter->saveState()` / `restoreState()`
- `closeEvent()` override에서 저장, `setupUi()` 끝에서 복원
- 첫 실행 시(키 없음) 기본값: 1000×700, splitter 비율 4:1 / 3:2

### 31.5 설정 파일 경로 해석

config 디렉토리를 다양한 방식으로 지정할 수 있도록 `AppSettings::resolveConfigDir()` 유틸 함수를 제공한다.

해석 우선순위:
1. CLI 인수: `--config-dir /path/`
2. 환경변수: `BOTMANAGER_CONFIG_DIR=/path/`
3. 기본값: `QCoreApplication::applicationDirPath() + "/config"`
